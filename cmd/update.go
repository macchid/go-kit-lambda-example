...
func main() {
	...
	lambda.StartHandlerWithContext(ctx, awslambda.NewHandler(
		transport.MakeLoggerMiddleware(&logger, "Endpoints::Update")(transport.MakeUpdateEndpoint(svc)),
...
func decodeAGProxyRequest(ctx context.Context, b []byte) (interface{}, error) {
	...
	var updateRequest transport.UpdateRequest
	err = json.NewDecoder(strings.NewReader(req.Body)).Decode(&updateRequest)
	if err != nil {
		return transport.UpdateRequest{}, err
	}

	updateRequest.Email = req.PathParameters["email"]

	return updateRequest, nil
}

func encodeAGProxyResponse(ctx context.Context, resp interface{}) ([]byte, error) {
	result := resp.(transport.UpdateResponse)
